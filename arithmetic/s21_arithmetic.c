...
/// @param index
/// @param bit
void s21_decimal_set_bit_big(s21_big_decimal *dst, int index, int bit) {
...
/// @param index
/// @return
int s21_decimal_get_bit_big(s21_big_decimal dst, int index) {
  int mask = 1u << (index % 32);
...
void s21_shift_big_dec_right(s21_big_decimal *dst, int num) {
  ...
  for (int k = 0; k < num; k++) {
    for (int i = 0; i < 7; i++) {
      buffer[i] = s21_decimal_get_bit_big(*dst, (i + 1) * 32);
    }
    for (int i = 0; i < 7; i++) {
      dst->bits[i] >>= 1;
      s21_decimal_set_bit_big(dst, (i + 1) * 32 - 1, buffer[i]);
    }
    dst->bits[7] >>= 1;
  }
}
...
int s21_shift_big_dec_left(s21_big_decimal *dst, int num) {
  ...
  for (int k = 0; k < num; k++) {
    for (int i = 0; i < 7; i++) {
      buffer[i] = s21_decimal_get_bit_big(*dst, (i + 1) * 32 - 1);
    }
    for (int i = 7; i > 0 && !error; i--) {
      if (s21_decimal_get_bit_big(*dst, 255)) error = 1;
      dst->bits[i] <<= 1;
      s21_decimal_set_bit_big(dst, i * 32, buffer[i - 1]);
    }
    dst->bits[0] <<= 1;
  }
  return error;
...
int s21_post_normalization(s21_big_decimal *result, int scale) {
  ...
  if (dop && scale == 0 && result->bits[3] == 0 && s21_decimal_get_bit_big(*result, 0))
    s21_decimal_set_bit_big(result, 0, 0);
...
void s21_find_highest_bit_decimal(s21_decimal v1, s21_decimal v2, int *bit_1,
  ...
  for (int i = 95; i >= 0 && (!(*bit_1) || !(*bit_2)); i--) {
    if (*bit_1 == 0 && s21_decimal_get_bit(v1, i)) *bit_1 = i;
    if (*bit_2 == 0 && s21_decimal_get_bit(v2, i)) *bit_2 = i;
  }
}
...
void s21_find_highest_bit_big_decimal(s21_big_decimal v1, s21_big_decimal v2,
  ...
  for (int i = 255; i >= 0 && (!(*bit_1) || !(*bit_2)); i--) {
    if (*bit_1 == 0 && s21_decimal_get_bit_big(v1, i)) *bit_1 = i;
    if (*bit_2 == 0 && s21_decimal_get_bit_big(v2, i)) *bit_2 = i;
  }
}
...
void s21_copy_decimal(s21_decimal *d1, s21_decimal d2) {
  for (int i = 127; i >= 0; i--) {
    *d1 = s21_decimal_set_bit(*d1, i, s21_decimal_get_bit(d2, i));
  }
}
...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (s21_decimal_sign(value_1) != s21_decimal_sign(value_2)) {
    int sign = 0;
    sign = s21_decimal_sign(value_1);
    value_1 = s21_decimal_abs(value_1);
    value_2 = s21_decimal_abs(value_2);
  ...
  if (error == 1 && s21_decimal_sign(*result)) error = 2;
  if (error) s21_decimal_init(result);
  return error;
}
...
void s21_add_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int res = 0, ovf = 0;
  for (int i = 0; i < 256; i++) {
    res = s21_decimal_get_bit_big(value_1, i) + s21_decimal_get_bit_big(value_2, i) + ovf;
    ovf = res / 2;
    s21_decimal_set_bit_big(result, i, res % 2);
  }
}
...
int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (s21_decimal_sign(value_1) && s21_decimal_sign(value_2)) {
    ...
    value_1 = value_2;
    value_2 = temporary1;
    value_1 = s21_decimal_set_bit(value_1, 127, 0);
    value_2 = s21_decimal_set_bit(value_2, 127, 0);
  }
  if (s21_decimal_sign(value_1) != s21_decimal_sign(value_2)) {
    s21_decimal_sign(value_1) ? sign_res = 1 : 1;
    value_1 = s21_decimal_set_bit(value_1, 127, 0);
    value_2 = s21_decimal_set_bit(value_2, 127, 0);
...
void s21_sub_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int tmp = 0, res = 0;
  for (int i = 0; i < 256; i++) {
    res = s21_decimal_get_bit_big(value_1, i) - s21_decimal_get_bit_big(value_2, i) - tmp;
    tmp = res < 0;
    res = abs(res);
    s21_decimal_set_bit_big(result, i, res % 2);
  }
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  s21_import_to_big_decimal(value_1, &v1);
  s21_import_to_big_decimal(value_2, &v2);
  if (s21_decimal_sign(value_1) != s21_decimal_sign(value_2)) s21_set_sign(result);
...
int s21_mul_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  for (int i = 0; i < 256 && !error; i++) {
    if (s21_decimal_get_bit_big(value_2, i)) {
  ...
  for (int i = 0; i < 96 && s21_is_big_decimal_not_empty(value_1);) {
    ...
    for (; diff >= 0 && s21_is_big_decimal_not_empty(value_1);) {
      if (s21_is_greater_big_decimal(value_2, value_1)) {
        s21_decimal_set_bit_big(&tmp, 0, 0);
      } else {
        s21_sub_big_decimal(value_1, value_2, &value_1);
        s21_decimal_set_bit_big(&tmp, 0, 1);
      }
      i++;
      diff--;
...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  int error = 0, sign = 0;
  if (s21_decimal_sign(value_1)) sign = 1;
  value_1 = s21_decimal_set_bit(value_1, 127, 0);
  value_2 = s21_decimal_set_bit(value_2, 127, 0);
  if (!s21_is_decimal_no_empty(value_2)) {
    error = 3;
  ...
  *result = s21_decimal_set_bit(*result, 127, sign);
  if (error == 1 && s21_decimal_sign(*result)) error = 2;
  if (error) s21_decimal_init(result);
  return error;
}
...
int s21_get_integer_part(s21_decimal value_1, s21_decimal value_2,
  ...
  for (int j = b1; j >= 0; j--) {
    s21_decimal_set_bit_big(&nvalue, j, !s21_decimal_get_bit_big(v2, j));
  }
  s21_add_big_decimal(nvalue, one, &nvalue);
  for (int j = b1; j >= 0; j--) {
    s21_shift_big_dec_left(&tmp_rem, 1);
    high_bit = s21_decimal_get_bit_big(tmp_rem, b1);
    s21_decimal_set_bit_big(&tmp_rem, b1 + 1, 0);
    s21_decimal_set_bit_big(&tmp_rem, 0, s21_decimal_get_bit_big(v1, b1));
    s21_shift_big_dec_left(&v1, 1);
    s21_decimal_set_bit_big(&v1, b1 + 1, 0);
    if (high_bit) {
    ...
    s21_decimal_set_bit_big(&tmp_rem, b1 + 1, 0);
    high_bit = s21_decimal_get_bit_big(tmp_rem, b1);
    s21_decimal_set_bit_big(&v1, 0, !high_bit);
  }
  if (scale < 0) {
