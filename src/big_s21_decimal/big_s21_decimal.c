...
big_s21_decimal big_s21_decimal_add_mantis(big_s21_decimal value_1, big_s21_decimal value_2) {
	big_s21_decimal res;
	bd_s21_decimal_init(&res);

	unsigned int buf = 0;
  	for (unsigned int bit = 0; bit < BD_SIZE_MANTIS; ++bit) {
		buf = s21_decimal_is_set_bit(value_1, bit) + s21_decimal_is_set_bit(value_2, bit) + buf;
...
big_s21_decimal big_s21_decimal_sub_manits(big_s21_decimal value_1, big_s21_decimal value_2) {
	big_s21_decima res;
	
	for (unsigned int bit = 0; bit < BD_SIZE_MANTIS; ++bit) {
		int sub = s21_decimal_is_set_bit(value_1, bit) - s21_decimal_is_set_bit(value_2, bit);
		if (sub == -1) {
...
void big_s21_add(big_s21_decimal value_1, big_s21_decimal value_2, big_s21_decimal *res) {
